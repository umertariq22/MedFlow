...
export default async function handler(req, res){
    if(req.method == "POST"){
        ...
        let query = `SELECT email, password FROM Patient WHERE email = '${req.body.email}'`;
        ...
        output.then(async function(result){
            ...
            query = `UPDATE Patient SET password = '${req.body.newpassword}' WHERE email = '${req.body.email}'`;
            ExecuteQuery(query);
            res.json(true);
        });
    }
